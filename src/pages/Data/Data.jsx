...
import css from '../css/Data.module.scss';
  ...
  useEffect(() => {
    const fetchData = async () => {
      const unsubscribe = onSnapshot(collection(db, "data"), (querySnapshot) => {
        ...
        // Calculate averages for every 20 data points
        const averagedData = [];
        for (let i = 0; i < cities.length; i += 1000) {
          ...
          averagedData.push({
            ...
            time: chunk[Math.floor(chunkSize / 2)].time // Use the middle time for the average data point
          });
        }
